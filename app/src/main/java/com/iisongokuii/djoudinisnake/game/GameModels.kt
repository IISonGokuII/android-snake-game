...
enum class GameMode(val displayName: String, val description: String, val color: Color) {
    ...
    TRIAL("Djoudini's Trial", "Hindernisse und extreme Geschwindigkeit.", Color(0xFFFF1744L))
}

enum class TrickType(val color: Color, val durationMs: Long) {
    GHOST(Color(0xAAFFFFFFL), 5000L), // Ghost: Transparent, kann durch sich selbst gleiten
    MIRROR(Color(0xFFFFD700L), 10000L) // Mirror: Invertierte Steuerung, aber 3x Punkte!
}
